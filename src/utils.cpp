...
std::vector<Vertex> triangulateRingsToTris(const std::vector<std::vector<Vertex>>& polygonRings, float zTop, float zBottom) {
    ...
    for (auto &ring : polygonRings) {
        std::vector<Point> r;
        for (auto &p : ring) r.push_back({p.x, p.y});
    ...
    for (auto &ring : polygonRings) for (auto &v : ring) flat.push_back({v.x, v.y, (float)zBottom});
    ...
    for (auto &v : flatTop) v.z = (float)zTop;
    for (size_t i=0;i<idx.size(); i+=3) {
...
std::vector<Vertex> generateSideTriangles(const std::vector<Vertex>& ring, float height) {
    ...
    for (size_t i=0;i<n;i++){
        ...
        // tri 1: b0, t0, t1
        side.push_back(b0); side.push_back(t0); side.push_back(t1);
        // tri 2: b0, t1, b1
        side.push_back(b0); side.push_back(t1); side.push_back(b1);
    }
    return side;
}
